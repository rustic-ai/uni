...
#[tokio::test]
async fn test_vectorized_union_distinct() -> anyhow::Result<()> {
    ...
    // 2. UNION DISTINCT (implicit)
    let query = "RETURN 'A' as c UNION RETURN 'A' as c";
    let mut parser = CypherParser::new(query)?;
    let ast = parser.parse()?;
    let plan = planner.plan(ast)?;
    let res = executor.execute(plan, &prop_manager, &params).await?;
    assert_eq!(res.len(), 1);
    assert_eq!(res[0].get("c").unwrap().as_str().unwrap(), "A");

    // 3. UNION ALL
    let query = "RETURN 'A' as c UNION ALL RETURN 'A' as c";
    let mut parser = CypherParser::new(query)?;
    let ast = parser.parse()?;
    let plan = planner.plan(ast)?;
    let res = executor.execute(plan, &prop_manager, &params).await?;
    ...
    // 4. UNION DISTINCT multiple columns
    let query = "RETURN 'A' as c1, 1 as c2 UNION RETURN 'A' as c1, 1 as c2";
    let mut parser = CypherParser::new(query)?;
    let ast = parser.parse()?;
    let plan = planner.plan(ast)?;
    let res = executor.execute(plan, &prop_manager, &params).await?;
    ...
    // 5. UNION DISTINCT mixed
    let query = "RETURN 'A' as c1, 1 as c2 UNION RETURN 'A' as c1, 2 as c2";
    let mut parser = CypherParser::new(query)?;
    let ast = parser.parse()?;
    let plan = planner.plan(ast)?;
    let res = executor.execute(plan, &prop_manager, &params).await?;
